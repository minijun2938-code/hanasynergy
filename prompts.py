...
**[은행 업무 맥락 고정 규칙]**
- 모든 예시/상황/프로젝트는 **실제 은행 업무**로 들어주세요.
- 금지 예시: "신제품개발"처럼 일반 제조/IT 스타트업 느낌의 예시는 쓰지 마세요.
- 대신 아래 카테고리에서만 예시를 선택해 구체적으로 작성하세요:
  - 영업/지점: 개인·기업 고객 상담, 여·수신(대출/예금) 상담, 고객 이슈 대응
  - 심사/리스크: 여신 심사, 리스크 평가, 한도/담보 검토, 리스크 커뮤니케이션
  - 준법/감사: 컴플라이언스 체크, 내부통제, AML/KYC, 모니터링
  - 상품/채널: 금융상품(예: 예금/대출/카드/WM) **출시/개편**, 모바일뱅킹/인터넷뱅킹 UX 개선, 프로세스 개선
  - 자산관리/WM: 포트폴리오 제안, 고객 성향 파악, 사후관리
  - 운영/백오피스: 전산/업무 프로세스, CS/민원 처리, 서류/결재 플로우

**[리포트 항목]**
1. 🧪 **종합 협업 시너지** (0-100점 점수 및 한줄 평)
2. ⚖️ **상호 보완적 업무 강점** (두 사람이 만났을 때 폭발적으로 늘어나는 역량)
3. 🎯 **전략적 업무 매칭** (함께 맡으면 최고의 성과를 낼 "은행 업무" 유형)
